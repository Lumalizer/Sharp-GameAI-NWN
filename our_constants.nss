...
string MyColor(object oObject = OBJECT_SELF) {
	...
	if (sColor != COLOR_RED && sColor != COLOR_BLUE) return "";
	return sColor;
}
...
string WpDoubler() { return "WP_" + DOUBLER; }
...
string ClaimerOf(string sAltar) {
	...
	if (!GetIsObjectValid(oAltar)) return "";
	string sOwner = GetLocalString(oAltar, "OWNER");
	if (sOwner != COLOR_RED && sOwner != COLOR_BLUE) return "";
	return sOwner;
}
...
object MyPortal(object oObject = OBJECT_SELF) {
	...
	string sPortal =
		"PORTAL_" + MyColor(oObject) + "_" + IntToString(GetLocalInt(oObject, "INDEX"));
...
int GetAltarScore(string sColor, string sAltar) {
	object oAltar = GetObjectByTag(sAltar);
	if (!GetIsObjectValid(oAltar)) return 0;
...
void SetTeamName(string sColor, string sName) {
	...
	if (!GetIsObjectValid(oBrazier)) return;
...
string GetTeamName(string sColor) {
	object oBrazier = GetObjectByTag("BRAZIER");
	if (!GetIsObjectValid(oBrazier)) return sColor;

	string sName = GetLocalString(oBrazier, "TEAM_" + sColor);
	if (sName == "") return sColor;
...
int SameTeam(object oTarget, object oMe = OBJECT_SELF) {
	...
	if (!GetIsObjectValid(oMe)) return FALSE;
...
int GetTeamID(string sColor) {
	object oBrazier = GetObjectByTag("BRAZIER");
	if (!GetIsObjectValid(oBrazier)) return 1;
...
int IsWizard(object oObject = OBJECT_SELF) {
	...
	if (iIndex == 1 || iIndex == 7) return TRUE;
	return FALSE;
}
...
int IsCleric(object oObject = OBJECT_SELF) {
	if (!GetIsObjectValid(oObject)) return FALSE;
	int iIndex = GetLocalInt(oObject, "INDEX");
	if (iIndex == 2 || iIndex == 6) return TRUE;
	return FALSE;
}
...
int IsFighter(object oObject = OBJECT_SELF) {
	if (!GetIsObjectValid(oObject)) return FALSE;
	int iIndex = GetLocalInt(oObject, "INDEX");
	if (iIndex == 3 || iIndex == 5) return TRUE;
	return FALSE;
}
